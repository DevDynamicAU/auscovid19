...
export default {
	...
	computed: {
		defaultDatasets() {
			...
			const QLD = this.chartData.filter( v => v.ProviceState == "Queensland").map( v => v.Active );
			const NSW = this.chartData.filter( v => v.ProviceState == "New South Wales").map( v => v.Active );
			const ACT = this.chartData.filter( v => v.ProviceState == "Australian Capital Territory").map( v => v.Active );
			const VIC = this.chartData.filter( v => v.ProviceState == "Victoria").map( v => v.Active );
			const TAS = this.chartData.filter( v => v.ProviceState == "Tasmania").map( v => v.Active );
			const NT  = this.chartData.filter( v => v.ProviceState == "Northern Territory").map( v => v.Active );
			const SA  = this.chartData.filter( v => v.ProviceState == "South Australia").map( v => v.Active );
			const WA  = this.chartData.filter( v => v.ProviceState == "Western Australia").map( v => v.Active );
