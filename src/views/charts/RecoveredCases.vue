		...
		defaultDatasets() {
			...
			for (const country of this.countries) {
				...
				if (this.mainCountry.toLowerCase() == "australia") {
						...
						const QLD = this.chartData.filter( v => v.ProvinceState == "Queensland").map( v => v.Recovered );
						const NSW = this.chartData.filter( v => v.ProvinceState == "New South Wales").map( v => v.Recovered );
						const ACT = this.chartData.filter( v => v.ProvinceState == "Australian Capital Territory").map( v => v.Recovered );
						const VIC = this.chartData.filter( v => v.ProvinceState == "Victoria").map( v => v.Recovered );
						const TAS = this.chartData.filter( v => v.ProvinceState == "Tasmania").map( v => v.Recovered );
						const NT  = this.chartData.filter( v => v.ProvinceState == "Northern Territory").map( v => v.Recovered );
						const SA  = this.chartData.filter( v => v.ProvinceState == "South Australia").map( v => v.Recovered );
						const WA  = this.chartData.filter( v => v.ProvinceState == "Western Australia").map( v => v.Recovered );
					...
					if (this.chartData.filter( v=> v.Country == country).length > 0) {
						const countryData = this.chartData.filter( v => v.Country == country)[0].Cases.map( v => v.Recovered );
